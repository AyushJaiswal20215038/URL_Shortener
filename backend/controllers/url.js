...
async function handleGenerateNewShortURL(req,res){
    try {
        ...
        if(!body.url) return res.status(200).json({ msg: 'url is required!!!'});
        ...
        return res.status(201).json({newurl:{_id: newURL._id,shortID,redirectURL:body.url},deletedURL,msg:"Successfully Created"});
...
async function handleRedirectURL(req,res){
    try {
        ...
        // console.log(entry);
        if(!entry)return res.status(200).json({msg: "Wrong shortID used"});
